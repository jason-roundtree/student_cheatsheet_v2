...
var rimraf = (0, _es6Promisify.promisify)(_rimraf.default);
var absoluteMatch = /^https?:\/\//i;
...
var _default = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (args, context) {
    ...
    var unattendedMode = flags.yes || flags.y;

    var defaultOutputDir = _path.default.resolve(_path.default.join(workDir, 'dist'));

    var outputDir = _path.default.resolve(args.argsWithoutOptions[0] || defaultOutputDir);

    var config = (0, _getConfig.default)(workDir, {
      env: 'production'
    });
    ...
    var envVars = _v.default.getSanityEnvVars({
      env: 'production',
      basePath: workDir
    });

    var envVarKeys = Object.keys(envVars);
    ...
    var compiler = (0, _server.getWebpackCompiler)(compilationConfig);
    var compile = (0, _es6Promisify.promisify)(compiler.run.bind(compiler));
    ...
    if (shouldDelete) {
      ...
      spin.text = "Clearing output folder (".concat(Date.now() - deleteStart, "ms)");
      spin.succeed();
    }
    ...
    try {
      // Compile the bundle
      var statistics = yield compile();
      var stats = statistics.toJson();

      if (stats.errors && stats.errors.length > 0) {
        throw new Error("Errors while building:\n\n".concat(stats.errors.join('\n\n')));
      }

      spin.text = "Building Sanity (".concat(stats.time, "ms)");
      ...
      var doc = yield (0, _server.getDocumentElement)(_objectSpread({}, compilationConfig, {
        hashes: chunkMap
      }), {
        scripts: ['vendor.bundle.js', 'app.bundle.js'].map(asset => {
          var assetPath = absoluteMatch.test(asset) ? asset : "js/".concat(asset);
          return {
            path: assetPath,
      ...
      yield _fsExtra.default.writeFile(_path.default.join(outputDir, 'index.html'), "<!doctype html>".concat(_server.ReactDOM.renderToStaticMarkup(doc))); // Print build output, optionally stats if requested

      bundle.stats.warnings.forEach(output.print);
      spin.text = "Building index document (".concat(Date.now() - indexStart, "ms)");
      spin.succeed();

      if (flags.stats) {
        output.print('\nLargest modules (unminified, uncompressed sizes):');
        (0, _sortModulesBySize.default)(bundle.stats.modules).slice(0, 10).forEach(module => output.print("[".concat((0, _filesize.default)(module.size), "] ").concat(module.name)));
      } // Now compress the JS bundles


      if (!compilationConfig.skipMinify) {
        spin = output.spinner('Minifying JavaScript bundles').start();
        var compressStart = Date.now();
        ...
        spin.text = "Minifying JavaScript bundles (".concat(Date.now() - compressStart, "ms)");
        spin.succeed();
