...
class BlockObject extends _react.default.Component {
  constructor() {
    ...
    _defineProperty(this, "handleDragOverOtherNode", event => {
      ...
      if (event.target instanceof HTMLElement) {
        ...
        for (var i = 0; i < keyNodes.length; i++) {
          var key = keyNodes[i].getAttribute('data-key');
          var closestBlock = editor.value.document.getClosestBlock(key); // eslint-disable-next-line max-depth

          if (!targetDOMNode && closestBlock) {
            targetDOMNode = (0, _slateReact.findDOMNode)(closestBlock);
          }
        }
    ...
    _defineProperty(this, "handleDragEnd", event => {
      ...
      var _this$props3 = this.props,
          ...
          editor = _this$props3.editor; // Return if this is our node

      if (!target || target.node.key === node.key) {
        return;
      }

      var targetIndex = target && target.node && editor.value.document.nodes.indexOf(target.node);
      var nodeIndex = editor.value.document.nodes.indexOf(node);
      var newIndex = target.position === 'before' ? targetIndex : targetIndex + 1;

      if (targetIndex > nodeIndex) {
        newIndex--;
      }

      editor.removeNodeByKey(node.key).applyOperation({
        type: 'insert_node',
        path: [newIndex],
        node: node
      }).moveToEndOfNode(node).focus();
    });
    ...
    _defineProperty(this, "renderMenuItem", item => {
      ...
      }, item.intent ? /*#__PURE__*/_react.default.createElement(_router.IntentLink, {
        intent: item.intent,
        params: item.params
      }, Icon && /*#__PURE__*/_react.default.createElement(Icon, null), item.title) : /*#__PURE__*/_react.default.createElement(_react.Fragment, null, Icon && /*#__PURE__*/_react.default.createElement(Icon, null), "\xA0", item.title));
    });
  ...
  render() {
    var _this$props10 = this.props,
        ...
        blockContentFeatures = _this$props10.blockContentFeatures,
        editor = _this$props10.editor,
        isSelected = _this$props10.isSelected,
