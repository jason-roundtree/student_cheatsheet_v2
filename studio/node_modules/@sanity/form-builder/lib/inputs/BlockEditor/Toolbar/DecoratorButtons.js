...
var _CollapsibleButtonGroup = _interopRequireDefault(require("./CollapsibleButtonGroup"));
...
function getIcon(type, schemaIcon) {
  switch (type) {
    case 'strong':
...
class DecoratorButtons extends _react.default.Component {
  constructor() {
    ...
    _defineProperty(this, "renderDecoratorButton", item => {
      var editor = this.props.editor;
      var icon = item.icon || item.blockEditor && item.blockEditor.icon;
      var Icon = icon || getIcon(item.value); // We must not do a click-event here, because that messes with the editor focus!

      var onAction = () => {
        this.handleClick(item);
      };

      var shortCut = _SetMarksOnKeyComboPlugin.keyMaps[item.value] ? "(".concat(_SetMarksOnKeyComboPlugin.keyMaps[item.value], ")") : '';
      var title = "".concat(item.title, " ").concat(shortCut);
      return /*#__PURE__*/_react.default.createElement("span", {
        className: _DecoratorButtons.default.buttonWrapper,
        key: item.value
      }, /*#__PURE__*/_react.default.createElement(_ToolbarClickAction.default, {
        onAction: onAction,
        editor: editor,
        key: "decoratorButton".concat(item.value)
  ...
  shouldComponentUpdate(nextProps) {
    var nextMarks = nextProps.editorValue.marks.map(mrk => mrk.type);
    var currentMarks = this.props.editorValue.marks.map(mrk => mrk.type);
  ...
  getItems() {
    ...
    return blockContentFeatures.decorators.map(decorator => _objectSpread({}, decorator, {
  ...
  render() {
    ...
    var icon = items[0].icon || items[0].blockEditor && items[0].blockEditor.icon;

    if (items.length > 0 && collapsed) {
      return /*#__PURE__*/_react.default.createElement(_CollapsibleButtonGroup.default, {
        icon: icon || getIcon(items[0].value)
