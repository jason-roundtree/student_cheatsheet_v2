...
function createWeightedSearch(types, client) {
  ...
  return function search(queryString) {
    ...
    var filters = ['_type in $__types', opts.includeDrafts === false && "!(_id in path('drafts.**'))", ...constraints.map(constraint => "(".concat(constraint.join('||'), ")")), filter ? "(".concat(filter, ")") : ''].filter(Boolean);
