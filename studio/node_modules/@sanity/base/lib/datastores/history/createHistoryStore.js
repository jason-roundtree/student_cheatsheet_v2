...
var mapRefNodes = (doc, mapFn) => jsonMap(doc, node => {
  return typeof node && typeof node === 'object' && typeof node._ref === 'string' ? mapFn(node) : node;
});
...
function restore(id, targetId, rev) {
  return (0, _rxjs.from)(getDocumentAtRevision(id, rev)).pipe((0, _operators.mergeMap)(documentAtRevision => {
    ...
    return _client.default.observable.fetch("{".concat(existingIdsQuery, "}")).pipe((0, _operators.map)(existingIds => removeMissingReferences(documentAtRevision, existingIds)));
  }), (0, _operators.map)(documentAtRevision => // Remove _updatedAt and create a new draft from the document at given revision
  _objectSpread({}, (0, _omit2.default)(documentAtRevision, '_updatedAt'), {
    _id: targetId
  })), (0, _operators.mergeMap)(restoredDraft => _client.default.observable.transaction().createOrReplace(restoredDraft).commit()));
}
